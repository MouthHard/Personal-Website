<template>
  <div class="search-bar">
    <div class="search-container">
      ...
      <div class="search-input-container">
        <input v-model="searchQuery" type="text" class="search-input" placeholder="搜索诗词、作者、名句..."
          @keyup.enter="handleSearch" @focus="handleFocus" @blur="handleBlur" />
    ...
    <div v-if="isFocused && !searchQuery" class="hot-searches">
      ...
      <span v-for="(tag, index) in hotTags" :key="index" class="hot-tag" @click="selectTag(tag)">
        {{ tag }}
      </span>
    </div>

    <div v-if="isFocused && searchQuery && suggestions.length > 0" class="search-suggestions">
      <div v-for="(item, index) in suggestions" :key="index" class="suggestion-item" @click="selectSuggestion(item)">
...
import { searchPoems } from '@/utils/Aphorism/categoryFilter';
import { hotTags, poems } from '@/constants/Aphorism';
import './index.scss';
