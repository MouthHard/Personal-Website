import { ScenerySpot, TravelActivity, RouteInfo, SeasonalScenery } from "../scenery.ts";

// 台湾省自然风光数据
const taiwanNatureSpots: ScenerySpot[] = [
  {
    id: "sunMoonLake",
    name: "日月潭",
    description: "台湾省的著名湖泊，以其美丽的湖光山色和独特的日月形状而闻名",
    image: "https://picsum.photos/id/1019/600/400",
    location: "台湾省南投县鱼池乡",
    type: "nature",
    bestTime: "春季",
    tags: ["自然", "湖泊", "风景", "文化"],
    rating: 4.8,
    landscapeType: "lake",
    landscapeCategory: ["水景观"],
    landformTags: ["湖泊景观"]
  },
  {
    id: "aliMountain",
    name: "阿里山",
    description: "台湾省的著名山脉，以其壮丽的自然风光、云海和铁路而闻名",
    image: "https://picsum.photos/id/1035/600/400",
    location: "台湾省嘉义县阿里山乡",
    type: "nature",
    bestTime: "春季",
    tags: ["自然", "山脉", "云海", "铁路"],
    rating: 4.9,
    landscapeType: "mountain",
    landscapeCategory: ["地理景观"],
    landformTags: ["山地景观"]
  },
  {
    id: "tarokoGorge",
    name: "太鲁阁峡谷",
    description: "台湾省的著名峡谷，以其壮丽的峡谷风光和大理石岩层而闻名",
    image: "https://picsum.photos/id/1036/600/400",
    location: "台湾省花莲县秀林乡",
    type: "nature",
    bestTime: "夏季",
    tags: ["自然", "峡谷", "大理石", "风景"],
    rating: 4.8,
    landscapeType: "canyon",
    landscapeCategory: ["地理景观"],
    landformTags: ["峡谷景观"]
  },
  {
    id: "kentingNationalPark",
    name: "垦丁国家公园",
    description: "台湾省的著名国家公园，以其美丽的海滩和丰富的生态资源而闻名",
    image: "https://picsum.photos/id/1037/600/400",
    location: "台湾省屏东县恒春镇",
    type: "nature",
    bestTime: "夏季",
    tags: ["自然", "海滩", "生态", "风景"],
    rating: 4.7,
    landscapeType: "park",
    landscapeCategory: ["生物景观"],
    landformTags: ["公园景观"]
  },
  {
    id: "yangmingshanNationalPark",
    name: "阳明山国家公园",
    description: "台湾省的著名国家公园，以其美丽的自然风光和温泉而闻名",
    image: "https://picsum.photos/id/1038/600/400",
    location: "台湾省台北市士林区",
    type: "nature",
    bestTime: "春季",
    tags: ["自然", "公园", "温泉", "风景"],
    rating: 4.6,
    landscapeType: "park",
    landscapeCategory: ["生物景观"],
    landformTags: ["公园景观"]
  },
  {
    id: "eastCoastScenicArea",
    name: "东部海岸风景区",
    description: "台湾省的著名风景区，以其壮丽的海岸线和自然风光而闻名",
    image: "https://picsum.photos/id/1039/600/400",
    location: "台湾省花莲县和台东县",
    type: "nature",
    bestTime: "夏季",
    tags: ["自然", "海岸", "风景", "户外活动"],
    rating: 4.7,
    landscapeType: "ocean",
    landscapeCategory: ["水景观"],
    landformTags: ["海洋景观"]
  },
  {
    id: "greenIsland",
    name: "绿岛",
    description: "台湾省的著名岛屿，以其美丽的海滩和海洋生态而闻名",
    image: "https://picsum.photos/id/1040/600/400",
    location: "台湾省台东县绿岛乡",
    type: "nature",
    bestTime: "夏季",
    tags: ["自然", "岛屿", "海滩", "海洋生态"],
    rating: 4.5,
    landscapeType: "island",
    landscapeCategory: ["地理景观"],
    landformTags: ["岛屿景观"]
  }
];

// 台湾省人文景观数据
const taiwanCultureSpots: ScenerySpot[] = [
  {
    id: "taipei101",
    name: "台北101",
    description: "台湾省的著名地标建筑，以其高耸的建筑和观景台而闻名",
    image: "https://picsum.photos/id/1044/600/400",
    location: "台湾省台北市信义区",
    type: "culture",
    bestTime: "四季皆宜",
    tags: ["历史", "文化", "建筑", "观景台"],
    rating: 4.8,
    landscapeType: "urban",
    landscapeCategory: ["人文景观"],
    landformTags: ["城市地标"]
  },
  {
    id: "nationalPalaceMuseum",
    name: "故宫博物院",
    description: "台湾省的著名博物馆，以其丰富的中国文物和艺术品而闻名",
    image: "https://picsum.photos/id/1045/600/400",
    location: "台湾省台北市士林区",
    type: "culture",
    bestTime: "四季皆宜",
    tags: ["历史", "文化", "博物馆", "艺术品"],
    rating: 4.9,
    landscapeType: "museum",
    landscapeCategory: ["人文景观"],
    landformTags: ["博物馆"]
  },
  {
    id: "longshanTemple",
    name: "龙山寺",
    description: "台湾省的著名寺庙，以其精美的建筑和重要的宗教地位而闻名",
    image: "https://picsum.photos/id/1046/600/400",
    location: "台湾省台北市万华区",
    type: "culture",
    bestTime: "四季皆宜",
    tags: ["历史", "文化", "寺庙", "宗教"],
    rating: 4.6,
    landscapeType: "temple",
    landscapeCategory: ["人文景观"],
    landformTags: ["宗教建筑"]
  },
  {
    id: "chiangKaiShekMemorialHall",
    name: "中正纪念堂",
    description: "台湾省的著名纪念建筑，以其宏伟的建筑和历史意义而闻名",
    image: "https://picsum.photos/id/1047/600/400",
    location: "台湾省台北市中正区",
    type: "culture",
    bestTime: "四季皆宜",
    tags: ["历史", "文化", "纪念堂", "建筑"],
    rating: 4.7,
    landscapeType: "historical",
    landscapeCategory: ["人文景观"],
    landformTags: ["纪念建筑"]
  },
  {
    id: "tainanAnpingOldStreet",
    name: "台南安平老街",
    description: "台湾省的著名历史街区，以其丰富的历史文化和传统建筑而闻名",
    image: "https://picsum.photos/id/1048/600/400",
    location: "台湾省台南市安平区",
    type: "culture",
    bestTime: "四季皆宜",
    tags: ["历史", "文化", "老街", "传统建筑"],
    rating: 4.5,
    landscapeType: "historical",
    landscapeCategory: ["人文景观"],
    landformTags: ["历史街区"]
  }
];

// 台湾省季节性数据
const taiwanSeasonalData: Record<string, SeasonalScenery> = {
  spring: {
    name: "春季",
    description: "春季的台湾，万物复苏，是游览阿里山、日月潭的最佳时节",
    image: "https://picsum.photos/id/1049/600/400",
    activities: ["阿里山赏樱", "日月潭游船", "阳明山公园赏花", "台北101参观"]
  },
  summer: {
    name: "夏季",
    description: "夏季的台湾，虽然炎热，但也是游览垦丁国家公园、绿岛的好去处",
    image: "https://picsum.photos/id/1050/600/400",
    activities: ["垦丁海滩游泳", "绿岛潜水", "太鲁阁峡谷徒步", "东部海岸风景区游览"]
  },
  autumn: {
    name: "秋季",
    description: "秋季的台湾，天高气爽，是游览太鲁阁峡谷、中正纪念堂的最佳时节",
    image: "https://picsum.photos/id/1051/600/400",
    activities: ["太鲁阁峡谷徒步", "中正纪念堂参观", "故宫博物院游览", "龙山寺参拜"]
  },
  winter: {
    name: "冬季",
    description: "冬季的台湾，气候温暖，是游览台北101、故宫博物院的好时节",
    image: "https://picsum.photos/id/1052/600/400",
    activities: ["台北101观景", "故宫博物院深度游", "龙山寺参拜", "台南安平老街漫步"]
  }
};

// 台湾省活动数据
const taiwanActivities: TravelActivity[] = [
  {
    id: "1",
    name: "阿里山赏樱",
    description: "在阿里山欣赏美丽的樱花，感受春季的气息",
    image: "https://picsum.photos/id/1053/600/400",
    category: "nature",
    location: "台湾省嘉义县阿里山乡",
    duration: "全天",
    difficulty: "中等",
    type: "户外活动"
  },
  {
    id: "2",
    name: "垦丁海滩游泳",
    description: "在垦丁国家公园的海滩游泳，享受阳光和海浪",
    image: "https://picsum.photos/id/1054/600/400",
    category: "nature",
    location: "台湾省屏东县恒春镇",
    duration: "半天",
    difficulty: "简单",
    type: "户外活动"
  },
  {
    id: "3",
    name: "故宫博物院参观",
    description: "参观台湾省的著名博物馆故宫博物院，欣赏丰富的中国文物和艺术品",
    image: "https://picsum.photos/id/1055/600/400",
    category: "culture",
    location: "台湾省台北市士林区",
    duration: "全天",
    difficulty: "简单",
    type: "文化体验"
  },
  {
    id: "4",
    name: "太鲁阁峡谷徒步",
    description: "在太鲁阁峡谷进行徒步旅行，欣赏壮丽的峡谷风光和大理石岩层",
    image: "https://picsum.photos/id/1056/600/400",
    category: "nature",
    location: "台湾省花莲县秀林乡",
    duration: "全天",
    difficulty: "中等",
    type: "户外活动"
  },
  {
    id: "5",
    name: "台湾美食之旅",
    description: "品尝台湾特色美食，如台湾卤肉饭、珍珠奶茶、台湾小笼包等",
    image: "https://picsum.photos/id/1057/600/400",
    category: "culture",
    location: "台湾省台北市中正区",
    duration: "全天",
    difficulty: "简单",
    type: "美食体验"
  }
];

// 台湾省路线数据
const taiwanRoutes: RouteInfo[] = [
  {
    id: "1",
    name: "台湾环岛之旅",
    title: "台湾环岛之旅",
    description: "环绕台湾岛游览，探索台湾的自然风光和人文景观",
    duration: "10天",
    difficulty: "中等",
    highlights: ["台北101", "故宫博物院", "阿里山", "日月潭", "太鲁阁峡谷", "垦丁国家公园"],
    spots: ["台北101", "故宫博物院", "阿里山", "日月潭", "太鲁阁峡谷", "垦丁国家公园"],
    tags: ["自然", "文化", "环岛"],
    image: "https://picsum.photos/id/1058/600/400",
    rating: 4.9,
    bestTime: "春季",
    traffic: "自驾+公共交通",
    suitableFor: "所有游客"
  },
  {
    id: "2",
    name: "台湾北部文化之旅",
    title: "台湾北部文化之旅",
    description: "探索台湾北部的历史文化和现代都市",
    duration: "5天",
    difficulty: "简单",
    highlights: ["台北101", "故宫博物院", "龙山寺", "中正纪念堂", "阳明山国家公园"],
    spots: ["台北101", "故宫博物院", "龙山寺", "中正纪念堂", "阳明山国家公园"],
    tags: ["历史", "文化", "都市"],
    image: "https://picsum.photos/id/1059/600/400",
    rating: 4.8,
    bestTime: "四季皆宜",
    traffic: "地铁+公交",
    suitableFor: "所有游客"
  }
];

// 台湾省非物质文化遗产数据
const taiwanIntangibleHeritage = [
  {
    id: "1",
    name: "台湾歌仔戏",
    description: "台湾省传统戏曲艺术，以其独特的表演形式和丰富的内容著称",
    level: "national",
    category: "传统戏剧",
    heritageType: "传统表演艺术",
    protectionUnit: "台湾省文化部门",
    tags: ["传统戏剧", "国家级非遗", "台湾文化"],
    icon: "🎭",
    image: "https://picsum.photos/id/1065/600/400",
    features: ["表演独特", "内容丰富", "历史悠久"]
  },
  {
    id: "2",
    name: "台湾传统木偶戏",
    description: "台湾省传统民间艺术，以其精美的木偶制作和独特的表演形式著称",
    level: "national",
    category: "传统戏剧",
    heritageType: "传统表演艺术",
    protectionUnit: "台湾省文化部门",
    tags: ["传统戏剧", "国家级非遗", "台湾文化"],
    icon: "🎭",
    image: "https://picsum.photos/id/1066/600/400",
    features: ["表演独特", "木偶精美", "历史悠久"]
  },
  {
    id: "3",
    name: "台湾传统茶文化",
    description: "台湾省传统饮食文化，以其独特的茶艺和茶道著称",
    level: "national",
    category: "传统技艺",
    heritageType: "传统手工艺",
    protectionUnit: "台湾省文化部门",
    tags: ["传统技艺", "国家级非遗", "台湾文化"],
    icon: "🍵",
    image: "https://picsum.photos/id/1067/600/400",
    features: ["茶艺独特", "茶道精深", "历史悠久"]
  },
  {
    id: "4",
    name: "台湾传统建筑工艺",
    description: "台湾省传统手工艺，以其精美的建筑工艺和独特的风格著称",
    level: "national",
    category: "传统技艺",
    heritageType: "传统手工艺",
    protectionUnit: "台湾省文化部门",
    tags: ["传统技艺", "国家级非遗", "台湾文化"],
    icon: "🏠",
    image: "https://picsum.photos/id/1068/600/400",
    features: ["工艺精湛", "风格独特", "历史悠久"]
  },
  {
    id: "5",
    name: "台湾传统节庆习俗",
    description: "台湾省传统民俗活动，以其独特的节庆仪式和丰富的文化内涵著称",
    level: "national",
    category: "民俗",
    heritageType: "民俗活动",
    protectionUnit: "台湾省文化部门",
    tags: ["民俗", "国家级非遗", "台湾文化"],
    icon: "🎉",
    image: "https://picsum.photos/id/1069/600/400",
    features: ["仪式独特", "文化内涵丰富", "历史悠久"]
  }
];

// 台湾省数据
export const taiwanData = {
  overview: {
    totalSpots: 12,
    natureSpots: 7,
    cultureSpots: 5,
    topAttractions: [
      {
        name: "阿里山",
        image: "https://picsum.photos/id/1035/600/400",
        description: "台湾省的著名山脉，以其壮丽的自然风光、云海和铁路而闻名"
      },
      {
        name: "日月潭",
        image: "https://picsum.photos/id/1019/600/400",
        description: "台湾省的著名湖泊，以其美丽的湖光山色和独特的日月形状而闻名"
      },
      {
        name: "台北101",
        image: "https://picsum.photos/id/1044/600/400",
        description: "台湾省的著名地标建筑，以其高耸的建筑和观景台而闻名"
      },
      {
        name: "故宫博物院",
        image: "https://picsum.photos/id/1045/600/400",
        description: "台湾省的著名博物馆，以其丰富的中国文物和艺术品而闻名"
      },
      {
        name: "太鲁阁峡谷",
        image: "https://picsum.photos/id/1036/600/400",
        description: "台湾省的著名峡谷，以其壮丽的峡谷风光和大理石岩层而闻名"
      }
    ]
  },
  spots: [...taiwanNatureSpots, ...taiwanCultureSpots],
  seasonal: [
    taiwanSeasonalData.spring,
    taiwanSeasonalData.summer,
    taiwanSeasonalData.autumn,
    taiwanSeasonalData.winter
  ],
  activities: taiwanActivities,
  routes: taiwanRoutes,
  intangibleHeritage: taiwanIntangibleHeritage
};

// 台湾省风俗文化数据
export const taiwanCustomsData = {
  activities: [
    {
      id: "1",
      name: "台湾歌仔戏表演体验",
      description: "欣赏台湾省传统戏曲艺术歌仔戏表演，感受其独特的表演形式和丰富的内容",
      image: "https://picsum.photos/id/1060/600/400",
      category: "culture",
      location: "台湾省台北市万华区",
      duration: "2-3小时",
      difficulty: "简单",
      type: "文化体验"
    },
    {
      id: "2",
      name: "台湾传统木偶戏表演体验",
      description: "欣赏台湾省传统民间艺术木偶戏表演，感受其精美的木偶制作和独特的表演形式",
      image: "https://picsum.photos/id/1061/600/400",
      category: "culture",
      location: "台湾省台南市安平区",
      duration: "2-3小时",
      difficulty: "简单",
      type: "文化体验"
    },
    {
      id: "3",
      name: "台湾传统茶文化体验",
      description: "体验台湾省传统茶文化，学习茶艺和茶道",
      image: "https://picsum.photos/id/1062/600/400",
      category: "culture",
      location: "台湾省台北市中正区",
      duration: "3-4小时",
      difficulty: "中等",
      type: "文化体验"
    },
    {
      id: "4",
      name: "台湾传统建筑工艺体验",
      description: "了解台湾省传统建筑工艺，参观传统建筑",
      image: "https://picsum.photos/id/1063/600/400",
      category: "culture",
      location: "台湾省台南市安平区",
      duration: "3-4小时",
      difficulty: "中等",
      type: "文化体验"
    },
    {
      id: "5",
      name: "台湾美食之旅",
      description: "品尝台湾特色美食，如台湾卤肉饭、珍珠奶茶、台湾小笼包等",
      image: "https://picsum.photos/id/1064/600/400",
      category: "culture",
      location: "台湾省台北市中正区",
      duration: "全天",
      difficulty: "简单",
      type: "美食体验"
    }
  ]
};

// 台湾民俗风情元素数据
export const taiwanFolkCustomsElements = [
  {
    name: "台湾歌仔戏",
    description: "台湾省传统戏曲艺术，以其独特的表演形式和丰富的内容著称",
    image: "https://picsum.photos/id/1070/600/400",
    tags: ["传统戏曲", "非遗文化", "闽南文化", "台湾特色", "表演艺术"]
  },
  {
    name: "台湾传统木偶戏",
    description: "台湾省传统民间艺术，以其精美的木偶制作和独特的表演形式著称",
    image: "https://picsum.photos/id/1071/600/400",
    tags: ["传统戏曲", "非遗文化", "民间艺术", "台湾特色", "雕刻工艺"]
  },
  {
    name: "台湾传统茶文化",
    description: "台湾省传统饮食文化，以其独特的茶艺和茶道著称",
    image: "https://picsum.photos/id/1072/600/400",
    tags: ["饮食文化", "非遗文化", "茶艺文化", "台湾特色", "高山茶特色"]
  },
  {
    name: "台湾传统建筑工艺",
    description: "台湾省传统手工艺，以其精美的建筑工艺和独特的风格著称",
    image: "https://picsum.photos/id/1073/600/400",
    tags: ["传统建筑", "非遗文化", "闽南建筑", "台湾特色", "建筑技艺"]
  },
  {
    name: "台湾传统节庆习俗",
    description: "台湾省传统民俗活动，以其独特的节庆仪式和丰富的文化内涵著称",
    image: "https://picsum.photos/id/1074/600/400",
    tags: ["民俗活动", "非遗文化", "节庆文化", "台湾特色", "文化传承"]
  }
];

// 台湾省文化节庆数据
export const taiwanFestivals = [
  {
    id: "1",
    name: "台湾灯会",
    description: "台湾举行的传统灯会活动，是台湾最具代表性的节庆活动之一",
    month: "2月",
    day: "1日",
    location: "台湾省台北市",
    isUnique: true
  },
  {
    id: "2",
    name: "台湾国际热气球嘉年华",
    description: "台湾举行的国际热气球嘉年华活动",
    month: "6月",
    day: "1日",
    location: "台湾省台东县鹿野乡",
    isUnique: true
  },
  {
    id: "3",
    name: "台湾阿里山樱花季",
    description: "台湾举行的阿里山樱花季活动",
    month: "3月",
    day: "1日",
    location: "台湾省嘉义县阿里山乡",
    isUnique: true
  },
  {
    id: "4",
    name: "台湾垦丁春呐音乐节",
    description: "台湾举行的垦丁春呐音乐节活动",
    month: "4月",
    day: "1日",
    location: "台湾省屏东县恒春镇",
    isUnique: true
  },
  {
    id: "5",
    name: "清明节祭扫",
    description: "传统祭祀祖先的节日",
    month: "4月",
    day: "5日",
    location: "台湾省各地",
    isUnique: false
  },
  {
    id: "6",
    name: "中秋节赏月",
    description: "传统节日，家人团聚赏月吃月饼",
    month: "9月",
    day: "21日",
    location: "台湾省各地",
    isUnique: false
  }
];

export default taiwanData;